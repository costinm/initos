...
To keep things simple - how to sign is kept separate, currently it's a form of minisign/signify but may change and have multiple formats/implementations.
...
can be used for other things besides rootfs. 

To reduce complexity I'm keeping only fs-verity/fs-crypt -it requires using an initrd, but still simpler than dm-verity/LUKS.